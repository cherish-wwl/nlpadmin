...
<script>
    ...
    import { getSolutionEntryInfo } from '@/api/solutions/solutionsManage'
    ...
    export default {
        ...
        props:["status","solutionId","entryInfo"],
        data () {
        ...
        methods:{
            ...
            submitForm (formName){
                ...
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        console.log("验证成功")
                        this.$emit("setVaildataEntryInfo",{vaild:true,info:this.serverInfo})
                        // return false;
                    } else {
                        this.$emit("setVaildataEntryInfo",{vaild:false,info:''})
                        // console.log('error submit!!');
                        return false;
                    }
                })
            },
            hasEntryInfoToInit(info){
                console.log("hasEntryInfoToInit")
                console.log(info)
                this.serverInfo = info
                 // 初始化
                if(this.serverInfo.accessWay == "002002"){
                    // 需要部署服务
                    this.rules = this.defaultRules 
                    this.tableData = this.defauleTableData
                    this.serverInfo.visitURL=''
        ...
        mounted (){
            ...
            getServerList().then( res =>{
                this.defauleTableData = res.data
            })
